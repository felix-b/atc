...
static async Task RunDaemon()
{
    ...
    async Task RunWhileExportingTelemetry()
    {
        ...
        var allFirs = InitializeFirs(telemetry, telemetryEnvironment);
        using var runLoop = BeginRunLoop(allFirs, telemetryEnvironment);
...
static RunLoop BeginRunLoop(FirEntry[] firs, CodePathEnvironment telemetryEnvironment)
{
    var allFirIds = firs.Select(f => f.Icao).ToArray();
    return new RunLoop(
        firs: firs,
        leaderFirIds: allFirIds, //offline mode
