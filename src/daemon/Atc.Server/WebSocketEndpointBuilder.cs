...
public class WebSocketEndpointBuilder
{
        ...
        where TDiscriminatorIn : Enum
    {
        ...
        public WebSocketEndpoint Create(IEndpointTelemetry telemetry, out IServiceTaskSynchronizer taskSynchronizer)
        {
            ...
            var messageSerializer = new ProtobufEnvelopeSerializer<TMessageIn>(telemetry);
            var connectionManager = new SocketConnectionManager(messageSerializer, operationDispatcher1, telemetry);
            
            var endpoint = new WebSocketEndpoint(_portNumber, _urlPath, connectionManager, telemetry);
            return endpoint;
        }
    }
}