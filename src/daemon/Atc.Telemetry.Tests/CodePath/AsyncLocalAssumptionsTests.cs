...
public class AsyncLocalAssumptionsTests
{
    ...
    [Test]
    public void ReturnsDifferentContextsInDifferentThreads()
    {
        var holder = new MyContextHolder();
    ...
    #if false 
    [Test] // this test's assumptions are wrong
    public async Task DisposableContextIsNotDisposedButGarbageCollected()
    {
        MyContext.FinalizeCount = 0;
        MyContext.DisposeCount = 0;
        
        var holder = new MyContextHolder();

        holder.SetNewContext();

        MyContext? childContext = null;
        ...
        MyContext.DisposeCount.Should().Be(0);
        MyContext.FinalizeCount.Should().Be(2);
    }
    #endif

    [Test]
    public async Task ExecutionContextDiffersInDifferentAsyncContexts()
    {
        var holder = new MyContextHolder();
        holder.SetNewContext();

        ExecutionContext? rootExecutionContext1 = ExecutionContext.Capture();
        
        ExecutionContext? childExecutionContext1 = null;
        ExecutionContext? childExecutionContext2 = null;

        Console.WriteLine($"root1:{Thread.CurrentThread.ManagedThreadId}");

        var task1 = async () => {
            await Task.Delay(100);
            holder.SetNewContext();
            childExecutionContext1 = ExecutionContext.Capture();
            Console.WriteLine($"child1:{Thread.CurrentThread.ManagedThreadId}");
        };

        var task2 = async () => {
            await Task.Delay(100);
            childExecutionContext2 = ExecutionContext.Capture();
            Console.WriteLine($"child2:{Thread.CurrentThread.ManagedThreadId}");
        };

        await Task.WhenAll(task1(), task2());

        Console.WriteLine($"root2:{Thread.CurrentThread.ManagedThreadId}");

        ExecutionContext? rootExecutionContext2 = ExecutionContext.Capture();

        rootExecutionContext1.Should().NotBeNull();
        rootExecutionContext2.Should().NotBeNull();
        childExecutionContext1.Should().NotBeNull();
        childExecutionContext2.Should().NotBeNull();

        (rootExecutionContext1 == rootExecutionContext2).Should().BeTrue();
        (childExecutionContext1 == rootExecutionContext1).Should().BeFalse();
        (childExecutionContext2 == rootExecutionContext1).Should().BeTrue();
        (childExecutionContext2 == childExecutionContext1).Should().BeFalse();
    }
