...
namespace Atc.Data.Sources.Tests.XP.Airports
{
    ...
    public class AirportReaderTests
    {
        ...
        public void ReadRealAirport_HUEN()
        {
            ...
            var airportRef = ReadSingleTestAirport(context, "apt_huen.dat");
            AssertAirport_HUEN(context, airportRef);
        }

        [Test]
        public void ReadRealAirport_HUEN_ForProfiling()
        {
            void FirstTime()
            {
                using var scope = AtcBufferContext.CreateEmpty(out var context);
        ...
        [Test]
        public void ReadRealAirport_HUEN_StreamRoundtrip()
        {
            using var stream = new MemoryStream();

            using (AtcBufferContext.CreateEmpty(out var contextBefore))
            {
                ReadSingleTestAirport(contextBefore, "apt_huen.dat");
                ((BufferContext) contextBefore).WriteTo(stream);
            }

            stream.Position = 0;

            using (AtcBufferContext.LoadFrom(stream, out var contextAfter))
            {
                var airportDataBuffer = contextAfter.GetBuffer<AirportData>(); 
                
                airportDataBuffer.RecordCount.Should().Be(1);
                AssertAirport_HUEN(contextAfter, airportDataBuffer.GetRecordZRef(recordIndex: 0));
            }
        }
